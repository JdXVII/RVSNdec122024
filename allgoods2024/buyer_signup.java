...
public class buyer_signup extends AppCompatActivity {

    private EditText login_email, login_password, firstName, lastName, phone;
    private Spinner provinces, cities, brgys;
    private Button sign_up;
    private ImageView profile, valid_id, btn_back;
    ...
    private ArrayAdapter<CharSequence> provincesAdapter, citiesAdapter, brgysAdapter;
    private EditText zipCode, zone;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        storageReference = FirebaseStorage.getInstance().getReference("buyer_images");

        // Initialize UI elements
        profile = findViewById(R.id.profile);
        valid_id = findViewById(R.id.valid_id);
        ...
        zone.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus) {
                animateView(v);
            }
        });
        ...
        provincesAdapter = ArrayAdapter.createFromResource(this,
                R.array.province_array, R.layout.custom_spinner_item);
        provincesAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        provinces.setAdapter(provincesAdapter);

        // Set up City Spinner (initially for Pangasinan)
        citiesAdapter = ArrayAdapter.createFromResource(this,
        ...
        valid_id.setOnClickListener(v -> openFileChooser(PICK_IMAGE_REQUEST));
        profile.setOnClickListener(v -> openFileChooser(PICK_PROFILE_IMAGE_REQUEST));

        // Handle register button click
        sign_up.setOnClickListener(v -> registerBuyer());

        // Handle back button click
        btn_back.setOnClickListener(v -> {
            Intent back = new Intent(buyer_signup.this, buyer_login.class);
            startActivity(back);
    ...
    private void updateBrgySpinner(int brgyArrayResource) {
        brgysAdapter = ArrayAdapter.createFromResource(this,
                brgyArrayResource, R.layout.custom_spinner_item);
        brgysAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        brgys.setAdapter(brgysAdapter);
    }
    ...
    private void registerBuyer() {
        String emailText = login_email.getText().toString().trim();
        String passwordText = login_password.getText().toString().trim();
        String firstNameText = firstName.getText().toString().trim();
        String lastNameText = lastName.getText().toString().trim();
        String phoneText = phone.getText().toString().trim();
        String provincesText = provinces.getSelectedItem().toString();
        String citiesText = cities.getSelectedItem().toString();
        String brgysText = brgys.getSelectedItem().toString();
        ...
        if (TextUtils.isEmpty(firstNameText) || TextUtils.isEmpty(lastNameText) || TextUtils.isEmpty(phoneText) ||
                imageUri == null || profileImageUri == null || TextUtils.isEmpty(zoneText) || TextUtils.isEmpty(zipCodeText)) {
                                        ...
                                        profileFileReference.putFile(profileImageUri).addOnCompleteListener(task2 -> {
                                            if (task2.isSuccessful()) {
                                                profileFileReference.getDownloadUrl().addOnSuccessListener(profileUri -> {
                                                    // Create buyer object with pending status
                                                    Buyer buyer = new Buyer(userId, emailText, firstNameText, lastNameText, phoneText, provincesText, citiesText, brgysText, profileUri.toString(), uri.toString(), "pending", zipCodeText, zoneText, passwordText); // Add passwordText here

                                                    // Save buyer object to database
                                                    databaseReference.child(userId).setValue(buyer)
                                                            .addOnCompleteListener(task3 -> {
                                                                if (task3.isSuccessful()) {
                                                                    Toast.makeText(buyer_signup.this, "Successfully signed up! Please verify your email and wait for admin approval.", Toast.LENGTH_LONG).show();
                                                                    auth.signOut();
                                                                    finish();
                                                                } else {
                                                                    Toast.makeText(buyer_signup.this, "Failed to save buyer info", Toast.LENGTH_SHORT).show();
                                                                }
                                                            });
                                                });
                                            } else {
                                                Toast.makeText(buyer_signup.this, "Failed to upload profile image", Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                    });
                                } else {
                                    Toast.makeText(buyer_signup.this, "Failed to upload valid id", Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                    } else {
                        Toast.makeText(buyer_signup.this, "Failed to register", Toast.LENGTH_SHORT).show();
                    }
                });
    }
        ...
        if (user != null) {
            user.sendEmailVerification()
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            Toast.makeText(buyer_signup.this, "Verification email sent to " + user.getEmail(), Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(buyer_signup.this, "Failed to send verification email", Toast.LENGTH_SHORT).show();
                        }
                    });
